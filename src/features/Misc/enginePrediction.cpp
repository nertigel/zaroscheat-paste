...
void CEnginePrediction::run(CUserCmd* cmd)
{
	...
	{
		m_pPredictionRandomSeed = *reinterpret_cast<int**>(g_Memory->findPattern("client_panorama.dll", "8B 0D ? ? ? ? BA ? ? ? ? E8 ? ? ? ? 83 C4 04") + 2); // A3 ? ? ? ? 66 0F 6E 86  / +1
		m_pSetPredictionPlayer = *reinterpret_cast<int**>(g_Memory->findPattern("client_panorama.dll", "89 35 ? ? ? ? F3 0F 10 48 20") + 2);
	}

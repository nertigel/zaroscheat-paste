...
{
	this->baseClient = ( CBaseClient* )g_InterfaceUtils->grabInterface( "client_panorama.dll", "VClient018" );
	this->gameEngine = ( CGameEngine* )g_InterfaceUtils->grabInterface( "engine.dll", "VEngineClient014" );
	this->gameUI = ( CGameUI* )g_InterfaceUtils->grabInterface( "client_panorama.dll", "GameUI011" );
	...
	this->clientEntity = ( IClientEntityList* )g_InterfaceUtils->grabInterface( "client_panorama.dll", "VClientEntityList003" );
	...
	this->prediction = ( CPrediction* )g_InterfaceUtils->grabInterface( "client_panorama.dll", "VClientPrediction001" );
	this->glow = *reinterpret_cast< CGlowObjectManager** >( g_Memory->findPattern( "client_panorama.dll", "0F 11 05 ? ? ? ? 83 C8 01" ) + 3 );
	...
	this->gameMovement = ( IGameMovement* )g_InterfaceUtils->grabInterface( "client_panorama.dll", "GameMovement001" );
	...
	this->weaponSystem = *reinterpret_cast< IWeaponSystem** >(g_Memory->findPattern( "client_panorama.dll", "8B 35 ? ? ? ? FF 10 0F B7 C0" ) + 2);

	this->moveHelper = **reinterpret_cast< IMoveHelper*** >( g_Memory->findPattern( "client_panorama.dll", "8B 0D ? ? ? ? 8B 46 08 68" ) + 2 );

	this->renderBeams = *reinterpret_cast< IViewRenderBeams** >( g_Memory->findPattern( "client_panorama.dll", "A1 ? ? ? ? FF 10 A1 ? ? ? ? B9" ) + 1 );
	...
	Globals::g_iModelBoneCounter = **reinterpret_cast< unsigned long** >( g_Memory->findPattern( "client_panorama.dll", "80 3D ? ? ? ? 00 74 16 A1" ) + 10 ); //80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81
}
